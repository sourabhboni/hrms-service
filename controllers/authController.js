...
const registerBusiness = async (req, res) => {
    ...
    try {
        ...
        // Hash the admin's password and create a new admin and organization
        const hashedPassword = await bcrypt.hash(password, 12);
        const admin = new Admin({ email, password: hashedPassword });
        const organization = new Organization({ name, domain, admin: admin._id });

        admin.organization = organization._id; // Link admin to organization

        // Save both the admin and the organization
